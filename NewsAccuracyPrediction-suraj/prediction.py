...
def fun_pred():
    ...
    test_set = pipeline_test(raw_test, bow_vectorizer,
    ...
    if mode == 'train':
        ...
        grads, _ = tf.clip_by_global_norm(
            tf.gradients(loss, tf_vars), clip_ratio)
...
def runModel(sess, keep_prob_pl, predict, features_pl, bow_vectorizer, tfreq_vectorizer, tfidf_vectorizer):
    ...
    userClaims = "test_stances_unlabeled.csv"
    userBodies = "test_bodies.csv"
    # parse that info
    raw_test = util.FNCData(userClaims, userBodies)
    # need more stuff for this
    test_set = util.pipeline_test(
        raw_test, bow_vectorizer, tfreq_vectorizer, tfidf_vectorizer)
...
def loadML():
    ...
    train_set, train_stances, bow_vectorizer, tfreq_vectorizer, tfidf_vectorizer = util.pipeline_train(
